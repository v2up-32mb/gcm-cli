...
选项:
  配置文件:
  -c, --config <file>          配置文件路径 (可选, 不指定则使用默认配置, 支持json或yaml格式)

  基本配置:
  -w, --worker <host>          Worker 地址
  -p, --port <number>          SOCKS5 监听端口 (默认: 1080)
  -u, --user-id <id>           用户ID (用于路径鉴权，默认: gcm)
  -l, --log-level <level>      日志级别: DEBUG/INFO/WARN/ERROR (默认: INFO)

  服务端配置:
      --proxy-ip <ip>          代理服务器IP (默认: 空, 使用服务端proxy ip)

  DNS配置:
  -d, --doh <url>              DoH 服务地址 (默认: https://v.recipes/dns-query)
      --no-doh                 禁用 DoH
  ...
  连接池配置:
      --no-warmup              禁用连接池预热 (默认: 启用)
      --no-reconnect           禁用断线自动重连 (默认: 启用)
      --no-dynamic-pool        禁用动态池大小调整 (默认: 启用)
...
示例:
  ...
  node gcm-single.cjs -w example.com -u myId -l DEBUG             # 完整配置示例
...
const DEFAULT_CONFIG = {
  ...
  relayIPs: [], // 中转节点IP字符串列表，逗号分隔，支持IP | IP:PORT | Domain | Domain:PORT格式
  enableDoH: true, // 是否启用 DoH 功能
  dohUrl: "https://v.recipes/dns-query", // DoH 服务地址
  ...
  heartbeatInterval: 15 * 1000, // 心跳间隔 (ms, 默认15秒)
  heartbeatTimeout: 1000, // 心跳超时时间 (ms, 默认1秒)
  enableTcpNoDelay: true, // 是否启用 TCP NoDelay 选项
      ...
      level === "DEBUG"
        ...
        : level === "INFO"
          ...
          : level === "WARN"
            ...
            : "E";
    console.log(
      `[${new Date().toLocaleTimeString()}] [${levelTag}] [${scope}] ${msg}`,
    );
  }
};
...
function loadConfig() {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (ext === ".yaml" || ext === ".yml") {
          if (!yaml) {
            throw new Error(
              "未安装 js-yaml 模块，无法解析 YAML 配置文件。请运行 npm install js-yaml",
            );
          }
  ...
  if (cliArgs.proxyIP) config.proxyIP = cliArgs.proxyIP;
  if (cliArgs.enableDoH !== null) config.enableDoH = cliArgs.enableDoH;
  if (cliArgs.enableMetrics !== null)
    config.enableMetrics = cliArgs.enableMetrics;
  if (cliArgs.metricsPort !== null) config.metricsPort = cliArgs.metricsPort;
  if (cliArgs.enablePoolWarmup !== null)
    config.enablePoolWarmup = cliArgs.enablePoolWarmup;
  if (cliArgs.enableAutoReconnect !== null)
    config.enableAutoReconnect = cliArgs.enableAutoReconnect;
  if (cliArgs.enableDynamicPool !== null)
    config.enableDynamicPool = cliArgs.enableDynamicPool;
  ...
  if (cliArgs.tunnelTimeout !== null)
    config.tunnelTimeout = cliArgs.tunnelTimeout;
  ...
  if (cliArgs.minPoolSize !== null)
    overrides.push(`min-pool=${cliArgs.minPoolSize}`);
  if (cliArgs.maxPoolSize !== null)
    overrides.push(`max-pool=${cliArgs.maxPoolSize}`);
  if (cliArgs.relayIPs) overrides.push(`relay=${cliArgs.relayIPs.join(",")}`);
  if (cliArgs.proxyIP) overrides.push(`proxy=${cliArgs.proxyIP}`);
  if (cliArgs.enableDoH === false) overrides.push("no-doh");
  if (cliArgs.enableMetrics === false) overrides.push("no-metrics");
  if (cliArgs.metricsPort !== null)
    overrides.push(`metrics-port=${cliArgs.metricsPort}`);
  ...
  if (cliArgs.tunnelTimeout !== null)
    overrides.push(`timeout=${cliArgs.tunnelTimeout / 1000}s`);
...
class FileLogger {
  constructor() {
    ...
    if (this.enabled) {
      ...
      tempLog(
        "FileLogger",
        `文件日志已启用: ${this.filePath} (最大${(this.maxSize / 1024 / 1024).toFixed(1)}MB, 保留${this.backupCount}个备份)`,
        "INFO",
      );
    } else {
    ...
    try {
      ...
      if (fs.existsSync(this.filePath)) {
        ...
        if (this.currentSize >= this.maxSize) {
          tempLog(
            "FileLogger",
            `初始化时检测到日志文件已满 (${(this.currentSize / 1024).toFixed(0)}KB)，执行轮转`,
            "DEBUG",
          );
          this.rotate();
        }
      ...
      tempLog(
        "FileLogger",
        `日志文件流已创建，当前大小: ${(this.currentSize / 1024).toFixed(0)}KB`,
        "DEBUG",
      );
    } catch (e) {
  ...
  rotate() {
    ...
    tempLog(
      "FileLogger",
      `日志轮转完成，耗时${elapsed}ms，轮转${rotated}个备份文件`,
      "DEBUG",
    );
  }
  ...
  close() {
    if (this.writeStream) {
      ...
      tempLog(
        "FileLogger",
        `关闭文件日志: 总写入${(stats.writtenBytes / 1024).toFixed(1)}KB，轮转${stats.rotatedCount}次`,
        "INFO",
      );
      this.writeStream.end();
      this.writeStream = null;
            ...
            res.on("end", () => {
              ...
              try {
                ...
                if (json.Answer && json.Answer.length > 0) {
                  const record = json.Answer.find(
                    (r) => r.type === (type === "A" ? 1 : 28),
                  );
                  if (record) {
                    logDebug(
                      "DoH",
                      `解析成功: ${domain} -> ${record.data} (${type}), 耗时${elapsed}ms`,
                    );
                    resolve(record.data);
                    return;
                  }
                ...
                logDebug(
                  "DoH",
                  `解析无结果: ${domain} (${type}), 耗时${elapsed}ms`,
                );
                resolve(null);
              } catch (e) {
...
class DNSCache {
  constructor() {
    ...
    logDebug(
      "DNSCache",
      `DNS缓存已初始化 (TTL: ${CONFIG.dnsCacheTTL / 1000}秒, 清理间隔: ${CONFIG.dnsCacheCleanupInterval / 1000}秒)`,
    );
  }
  ...
  get(domain, type = "A") {
    ...
    if (entry) {
      if (Date.now() < entry.expiresAt) {
        ...
        logDebug(
          "DNSCache",
          `缓存命中: ${domain} (${type}) -> ${entry.ip} (TTL:${ttl}s)`,
        );
        return entry.ip;
      } else {
        // 过期，删除
  ...
  set(domain, type, ip) {
    ...
    logDebug(
      "DNSCache",
      `缓存添加: ${domain} (${type}) -> ${ip} (TTL:${CONFIG.dnsCacheTTL / 1000}s)`,
    );
  }
  ...
  cleanup() {
    ...
    if (cleaned > 0 || beforeSize > 0) {
      ...
      logDebug(
        "DNSCache",
        `清理完成: 清除${cleaned}条过期缓存, 剩余${this.cache.size}条, 耗时${elapsed}ms`,
      );
    }
  ...
  async warmup(domains) {
    ...
    logInfo(
      "DNSCache",
      `预热完成: 成功${successCount}条, 当前缓存${this.cache.size}条, 耗时${elapsed}ms`,
    );
  }
...
class RelayManager {
  constructor(relayList) {
    ...
    logDebug(
      "Relay",
      `中转节点管理器已初始化，配置节点数: ${relayList.length}`,
    );
  }
  ...
  async init() {
      ...
      if (net.isIP(host)) {
        ...
        try {
          ...
          if (addresses.length > 0) {
            logDebug(
              "Relay",
              `域名 ${host} 解析到 ${addresses.length} 个 IP 地址`,
            );
            const domainCandidates = addresses.map((a) => ({
              ip: a.address,
              port,
              source: host,
            }));
            const testedDomainCandidates =
              await this.batchTestLatency(domainCandidates);

            const bestOfDomain = testedDomainCandidates.slice(0, 2);
            candidateNodes.push(...bestOfDomain);
            logDebug(
              "Relay",
              `域名 ${host} 优选了 ${bestOfDomain.length} 个节点`,
            );
          } else {
    ...
    if (this.optimalRelays.length > 0) {
      logInfo(
        "Relay",
        `初始化完成: 有效节点${this.optimalRelays.length}个 (过滤${filteredCount}个高延迟节点), 耗时${elapsed}ms`,
      );
      logInfo(
        "Relay",
        `优选节点列表 (Top ${Math.min(5, this.optimalRelays.length)}):`,
      );
      this.optimalRelays.slice(0, 5).forEach((r, i) => {
        logInfo(
          "Relay",
          `  [${i + 1}] ${r.ip}:${r.port} (${r.latency}ms, 分数:${r.score}) [来自: ${r.source}]`,
        );
      });
    } else {
      logWarn(
        "Relay",
        `未找到可用中转节点 (测速${results.length}个，全部超过${CONFIG.relayMaxLatency}ms阈值)，降级为直连模式`,
      );
    }
    ...
    logDebug(
      "Relay",
      `后台备用重评已启动 (间隔:${CONFIG.relayRescoreInterval / 1000}秒)`,
    );
  }
  ...
  startRescoring() {
    this.rescoreTimer = setInterval(async () => {
      ...
      logInfo(
        "Relay",
        `开始全面重新评分 ${this.optimalRelays.length} 个节点...`,
      );
      ...
      logInfo(
        "Relay",
        `重新评分完成: 有效${this.optimalRelays.length}个 (移除${removed}个), 耗时${elapsed}ms`,
      );
      this.logTopRelays();
   ...
   */
  resortByScore() {
    const before = this.optimalRelays.map((r) => ({
      ip: r.ip,
      port: r.port,
      score: r.score,
    }));
  ...
  reportFailure(ip, port) {
    const idx = this.optimalRelays.findIndex(
    ...
    if (idx !== -1) {
      ...
      logDebug(
        "Relay",
        `节点 ${ip}:${port} 失败报告: ${this.optimalRelays[idx].failureCount}/${CONFIG.relayFailureThreshold}, 分数: ${prevScore} -> ${this.optimalRelays[idx].score}`,
      );

      if (
        this.optimalRelays[idx].failureCount >= CONFIG.relayFailureThreshold
      ) {
        this.totalRemovedCount++;
        logWarn(
          "Relay",
          `节点 ${ip}:${port} 连续失败 ${CONFIG.relayFailureThreshold} 次，已移除 (累计移除: ${this.totalRemovedCount})`,
        );
  ...
  logTopRelays() {
    ...
    logInfo(
      "Relay",
      `当前最优节点 (Top ${Math.min(5, this.optimalRelays.length)}):`,
    );
    this.optimalRelays.slice(0, 5).forEach((r, i) => {
      logInfo(
        "Relay",
        `  [${i + 1}] ${r.ip}:${r.port} (延迟:${r.latency}ms, 失败:${r.failureCount}, 分数:${r.score}) [来自: ${r.source}]`,
      );
    });
  }
  ...
  async forceRescore() {
    ...
    const now = Date.now();
    if (
      this.lastForceRescoreTime &&
      now - this.lastForceRescoreTime < CONFIG.relayForceRescoreCooldown
    ) {
    ...
    logWarn(
      "Relay",
      `触发强制重新评分 (原最优: ${beforeBest?.ip}:${beforeBest?.port} ${beforeBest?.latency}ms)...`,
    );
    ...
    if (beforeBest && afterBest) {
      logInfo(
        "Relay",
        `最优节点: ${beforeBest.ip}:${beforeBest.port}(${beforeBest.latency}ms) -> ${afterBest.ip}:${afterBest.port}(${afterBest.latency}ms)`,
      );
    }
  ...
  getStats() {
    return {
      ...
      avgLatency:
        this.optimalRelays.length > 0
          ? Math.round(
              this.optimalRelays.reduce((sum, r) => sum + r.latency, 0) /
                this.optimalRelays.length,
            )
          : 0,
      bestLatency:
        this.optimalRelays.length > 0 ? this.optimalRelays[0].latency : 0,
      worstLatency:
        this.optimalRelays.length > 0
          ? this.optimalRelays[this.optimalRelays.length - 1].latency
          : 0,
    };
  }
...
class ConnectionPool extends EventEmitter {
  constructor(relayManager) {
    ...
    this.stats = {
      ...
      maxSendRate: 0, // 最大发送速率 (B/s)
      maxRecvRate: 0, // 最大接收速率 (B/s)
    ...
    // 启动速率统计定时器 (1秒)
    this.rateCheckTimer = setInterval(() => this.updateRateStats(), 1000);

    logDebug(
      "Pool",
      `连接池已初始化 (Min:${CONFIG.minPoolSize}, Max:${CONFIG.maxPoolSize}, 多路复用:${CONFIG.enableMultiplex ? "开启" : "关闭"})`,
    );
    ...
    if (CONFIG.enableDynamicPool) {
      this.dynamicPoolTimer = setInterval(
        () => this.adjustPoolSize(),
        CONFIG.dynamicPoolInterval,
      );
    }
  }
  ...
  async warmup() {
    ...
    logInfo(
      "Pool",
      `开始预热连接池，目标: ${this.currentMinPoolSize} 个连接...`,
    );
  ...
  async initializeRelay() {
    ...
    try {
      ...
      if (this.currentRelay) {
        logInfo(
          "Pool",
          `当前中转节点: ${this.currentRelay.ip}:${this.currentRelay.port} (${this.currentRelay.latency}ms)`,
        );
      } else {
  ...
  scheduleReconnect(reason) {
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      const currentSize =
        this.pool.length + this.activeConnections + this.pendingConnections;
  ...
  logStats() {
    ...
    if (total > 0) {
      logDebug(
        "Stats",
        `连接池状态: 空闲 ${this.pool.length} | 活跃 ${this.activeConnections} | 建立中 ${this.pendingConnections} | 等待队列 ${this.requestQueue.length}`,
      );
      ...
      if (allConnections.length > 0) {
        // 按 RTT 排序
        allConnections.sort(
          (a, b) => (a.rtt || Infinity) - (b.rtt || Infinity),
        );

        const rttList = allConnections
          .map((item) => {
    ...
    if (Date.now() % 30000 < 10000) {
      ...
      logInfo(
        "Stats",
        `DNS缓存: ${dnsStats.size}条 | 命中率: ${dnsStats.hitRate} | 命中:${dnsStats.hits} 未命中:${dnsStats.misses}`,
      );
      ...
      if (relayStats.totalNodes > 0) {
        logInfo(
          "Stats",
          `中转节点: ${relayStats.totalNodes}个 | 平均延迟:${relayStats.avgLatency}ms | 最佳:${relayStats.bestLatency}ms | 最差:${relayStats.worstLatency}ms`,
        );
      }
      ...
      if (CONFIG.enableStats && this.stats.requests > 0) {
        ...
        logInfo(
          "Stats",
          `请求: ${this.stats.requests}次 | 成功率: ${successRate}% | 平均响应: ${avgResponseTime}ms | 超时: ${this.stats.timeouts}次 | 上传: ${(this.stats.bytesSent / 1024).toFixed(1)}KB | 下载: ${(this.stats.bytesReceived / 1024).toFixed(1)}KB | 运行: ${uptime}秒`,
        );
      }
    }
  }
  ...
  updateRateStats() {
    const now = Date.now();
    const timeDiff = (now - this.stats.lastRateCheckTime) / 1000; // seconds
    if (timeDiff < 0.1) return; // 避免时间间隔过小

    const totalTraffic = this.calculateTotalTraffic();
    const currentSent = totalTraffic.sent;
    const currentReceived = totalTraffic.received;

    const sentDiff = currentSent - this.stats.lastBytesSent;
    const recvDiff = currentReceived - this.stats.lastBytesReceived;

    const currentSendRate = (sentDiff > 0 ? sentDiff : 0) / timeDiff;
    const currentRecvRate = (recvDiff > 0 ? recvDiff : 0) / timeDiff;

    if (currentSendRate > this.stats.maxSendRate) {
      this.stats.maxSendRate = currentSendRate;
    }
    if (currentRecvRate > this.stats.maxRecvRate) {
      this.stats.maxRecvRate = currentRecvRate;
    }

    this.stats.lastBytesSent = currentSent;
    this.stats.lastBytesReceived = currentReceived;
    this.stats.lastRateCheckTime = now;
  }
  ...
  /**
   * 计算总流量 (累计关闭连接流量 + 当前活跃连接流量)
   */
  calculateTotalTraffic() {
    let sent = this.stats.accumulatedBytesSent;
    let received = this.stats.accumulatedBytesReceived;

    const countSocket = (ws) => {
      if (ws && ws._socket) {
        sent += ws._socket.bytesWritten || 0;
        received += ws._socket.bytesRead || 0;
      }
    };

    // 统计空闲池中的连接
    this.pool.forEach((item) => countSocket(item.ws));
    // 统计活跃连接
    this.activeConnectionsMap.forEach((item, ws) => countSocket(ws));
  ...
  async maintainPool() {
    ...
    if (currentSize < this.currentMinPoolSize) {
      ...
      currentSize < CONFIG.maxPoolSize
    ) {
      // 如果有积压请求且未达最大上限，激进创建
  ...
  adjustPoolSize() {
    ...
    if (utilizationRatio > CONFIG.dynamicPoolHighThreshold) {
      ...
      activeRatio < CONFIG.dynamicPoolLowThreshold &&
      this.currentMinPoolSize > CONFIG.dynamicPoolMinSize
    ) {
      // 低负载：减少连接池大小
      newSize = Math.max(
    ...
    if (newSize !== this.currentMinPoolSize) {
      logInfo(
        "DynamicPool",
        `调整 minPoolSize: ${this.currentMinPoolSize} -> ${newSize} (${reason})`,
      );
  ...
  cullOldConnections() {
    ...
    if (toRemove.size > 0) {
      ...
      logDebug(
        "Pool",
        `清理过期连接: 清除${toRemove.size}个 (${beforeSize} -> ${this.pool.length})`,
      );
    }
  }
  ...
  async createConnection(reason = "") {
    ...
    if (currentSize >= CONFIG.maxPoolSize) {
      logDebug(
        "Pool",
        `连接池已满 (${currentSize}/${CONFIG.maxPoolSize})，跳过创建: ${reason}`,
      );
      return;
    }
    ...
    const relayInfo = relay ? `${relay.ip}:${relay.port}` : "Direct";

    if (relay) {
      // 支持 IPv6 字面量在 URL 中
      const ipStr = net.isIPv6(relay.ip) ? `[${relay.ip}]` : relay.ip;
      url = `wss://${ipStr}:${relay.port}${path}`;
      logDebug("Pool", `创建连接 (${reason}) -> 中转: ${relayInfo}${path}`);
    } else {
      url = `wss://${CONFIG.workerHost}${path}`;
      logDebug(
        "Pool",
        `创建连接 (${reason}) -> 直连: ${CONFIG.workerHost}${path}`,
      );
    }
    ...
    try {
      ...
      return new Promise((resolve) => {
        ws.on("open", () => {
          ...
          logDebug(
            "Pool",
            `新连接 [${connectionId}] 已就绪 (${reason}), 握手延迟: ${lat}ms`,
          );
        ...
        ws.on("error", (e) => {
          ...
          logDebug(
            "Pool",
            `连接失败 [${connectionId}] (${reason}): ${e.message}`,
          );
          ...
          this.relayManager
            ...
            .then((success) => {
              if (success && this.relayManager.getCurrentBest()) {
                ...
                this.currentRelay = this.relayManager.getCurrentBest();
                const changed =
                  oldRelay?.ip !== this.currentRelay.ip ||
                  oldRelay?.port !== this.currentRelay.port;
                if (changed) {
                  logInfo(
                    "Pool",
                    `已切换中转节点: ${oldRelay?.ip}:${oldRelay?.port} -> ${this.currentRelay.ip}:${this.currentRelay.port} (${this.currentRelay.latency}ms)`,
                  );
                }
              }
            })
        ...
        ws.on("socket", (socket) => {
          if (socket) {
            ...
            // 监听 socket 关闭事件以累积流量统计
            socket.on("close", () => {
              this.stats.accumulatedBytesSent += socket.bytesWritten || 0;
              this.stats.accumulatedBytesReceived += socket.bytesRead || 0;
            });
          }
        });
      });
    ...
    if (CONFIG.enableMultiplex) {
      for (const [ws, item] of this.activeConnectionsMap) {
        ...
        if (
          ...
          item.streams < CONFIG.maxStreamsPerConnection
        ) {
          // 复用这个活跃连接
          item.streams++;
...
class MetricsServer {
  ...
  start() {
    ...
    this.server = http.createServer((req, res) => {
      ...
      if (req.url === "/metrics") {
        ...
        logDebug("Metrics", `Dashboard 请求来自: ${clientAddr}`);
        const monitorPath = path.join(__dirname, "monitor.html");
        if (fs.existsSync(monitorPath)) {
          res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
          fs.createReadStream(monitorPath).pipe(res);
        } else {
          res.writeHead(404, { "Content-Type": "text/plain; charset=utf-8" });
          res.end(`未找到 monitor.html 文件 (路径: ${monitorPath})`);
        }
    ...
    this.server.listen(CONFIG.metricsPort, () => {
      logInfo(
        "Metrics",
        `监听端口: http://0.0.0.0:${CONFIG.metricsPort}/metrics`,
      );
      logInfo("Metrics", `端点: /metrics (Prometheus), /health (健康检查)`);
      logInfo("Metrics", `面板: /monitor`);
    });
  ...
  generateMetrics() {
    ...
    lines.push("# HELP gcm_pool_created_total 已创建连接总数");
    lines.push("# TYPE gcm_pool_created_total counter");
    lines.push(`gcm_pool_created_total ${this.pool.stats.createdConnections}`);

    lines.push("# HELP gcm_pool_closed_total 已关闭连接总数");
    lines.push("# TYPE gcm_pool_closed_total counter");
    lines.push(`gcm_pool_closed_total ${this.pool.stats.closedConnections}`);

    // DNS 缓存指标
    const dnsStats = this.dnsCache.getStats();
    const totalDnsRequests = dnsStats.hits + dnsStats.misses;

    lines.push("# HELP gcm_dns_requests_total DNS总请求数");
    lines.push("# TYPE gcm_dns_requests_total counter");
    lines.push(`gcm_dns_requests_total ${totalDnsRequests}`);
    ...
    lines.push("# HELP gcm_dns_cache_hit_rate DNS缓存命中率");
    lines.push("# TYPE gcm_dns_cache_hit_rate gauge");
    lines.push(`gcm_dns_cache_hit_rate ${dnsStats.hitRate}`);
    ...
    lines.push("# HELP gcm_relay_removed_nodes 已移除中转节点总数");
    lines.push("# TYPE gcm_relay_removed_nodes counter");
    lines.push(`gcm_relay_removed_nodes ${relayStats.totalRemovedCount}`);
    ...
    if (CONFIG.enableStats) {
      ...
      lines.push("# HELP gcm_requests_success_total 成功请求数");
      lines.push("# TYPE gcm_requests_success_total counter");
      lines.push(`gcm_requests_success_total ${stats.successes}`);

      lines.push("# HELP gcm_requests_failure_total 失败请求数");
      lines.push("# TYPE gcm_requests_failure_total counter");
      lines.push(`gcm_requests_failure_total ${stats.failures}`);

      lines.push("# HELP gcm_requests_timeout_total 超时请求数");
      lines.push("# TYPE gcm_requests_timeout_total counter");
      lines.push(`gcm_requests_timeout_total ${stats.timeouts}`);
      ...
      lines.push(
        `gcm_request_duration_avg ${stats.avgResponseTime.toFixed(2)}`,
      );

      lines.push("# HELP gcm_request_duration_min 最小请求响应时间(毫秒)");
      lines.push("# TYPE gcm_request_duration_min gauge");
      lines.push(
        `gcm_request_duration_min ${stats.minResponseTime === Infinity ? 0 : stats.minResponseTime}`,
      );
      ...
      const totalTraffic = this.pool.calculateTotalTraffic();
      lines.push(`gcm_bytes_sent_total ${totalTraffic.sent}`);

      lines.push("# HELP gcm_bytes_received_total 总接收字节数");
      lines.push("# TYPE gcm_bytes_received_total counter");
      lines.push(`gcm_bytes_received_total ${totalTraffic.received}`);
      ...
      const sendRateAvg = uptimeSec > 0 ? stats.bytesSent / uptimeSec : 0;
      const recvRateAvg = uptimeSec > 0 ? stats.bytesReceived / uptimeSec : 0;

      lines.push("# HELP gcm_rate_send_avg 平均发送速率(B/s)");
      lines.push("# TYPE gcm_rate_send_avg gauge");
      lines.push(`gcm_rate_send_avg ${sendRateAvg.toFixed(2)}`);

      lines.push("# HELP gcm_rate_send_max 最大发送速率(B/s)");
      lines.push("# TYPE gcm_rate_send_max gauge");
      lines.push(`gcm_rate_send_max ${stats.maxSendRate.toFixed(2)}`);

      lines.push("# HELP gcm_rate_recv_avg 平均接收速率(B/s)");
      lines.push("# TYPE gcm_rate_recv_avg gauge");
      lines.push(`gcm_rate_recv_avg ${recvRateAvg.toFixed(2)}`);

      lines.push("# HELP gcm_rate_recv_max 最大接收速率(B/s)");
      lines.push("# TYPE gcm_rate_recv_max gauge");
      lines.push(`gcm_rate_recv_max ${stats.maxRecvRate.toFixed(2)}`);
    }

    // Per-connection metrics
    const allConns = [];
    this.pool.pool.forEach((c) => allConns.push({ ...c, state: "idle" }));
    for (const [ws, c] of this.pool.activeConnectionsMap) {
      allConns.push({ ...c, state: "active" });
    }

    if (allConns.length > 0) {
      lines.push("# HELP gcm_conn_rtt 连接RTT(ms)");
      lines.push("# TYPE gcm_conn_rtt gauge");
      lines.push("# HELP gcm_conn_stream_count 连接并发流数");
      lines.push("# TYPE gcm_conn_stream_count gauge");
      lines.push("# HELP gcm_conn_bytes_sent 连接发送字节数");
      lines.push("# TYPE gcm_conn_bytes_sent gauge");
      lines.push("# HELP gcm_conn_bytes_received 连接接收字节数");
      lines.push("# TYPE gcm_conn_bytes_received gauge");

      for (const c of allConns) {
        const relayVal = c.relay || "unknown";
        const labels = `ws_id="${c.connectionId}",relay="${relayVal}",state="${c.state}"`;

        lines.push(`gcm_conn_rtt{${labels}} ${c.rtt}`);
        lines.push(`gcm_conn_stream_count{${labels}} ${c.streams || 0}`);

        if (c.ws && c.ws._socket) {
          lines.push(
            `gcm_conn_bytes_sent{${labels}} ${c.ws._socket.bytesWritten || 0}`,
          );
          lines.push(
            `gcm_conn_bytes_received{${labels}} ${c.ws._socket.bytesRead || 0}`,
          );
        }
      }
    }
...
class Socks5Server {
  ...
  start() {
    ...
    this.server.on("error", (e) =>
      logError("Socks5", `启动失败: ${e.message}`),
    );
      ...
      try {
        if (state === "AUTH") {
          ...
          if (addrType === 0x01) {
            ...
            logDebug(
              "Socks5",
              `IPv4 请求 (${clientAddr}): ${targetAddr}:${targetPort}`,
            );
            ...
            logDebug(
              "Socks5",
              `IPv6 请求 (${clientAddr}): ${targetAddr}:${targetPort}`,
            );
          }
  ...
  async createTunnel(clientSocket, host, port, originalTarget = null) {
    ...
    try {
      ...
      logInfo(
        "Proxy",
        `新请求 -> ${targetToLog}:${port} | WS[${connectionId}] Stream[${streamId}]`,
      );
      ...
      timeoutTimer = setTimeout(() => {
        logWarn(
          "Tunnel",
          `隧道超时 (${CONFIG.tunnelTimeout}ms): ${host}:${port}`,
        );
        // 记录请求超时
        if (CONFIG.enableStats) {
...
async function main() {
  ...
  logInfo(
    "System",
    `DoH: ${CONFIG.enableDoH ? "开启" : "关闭"} (${CONFIG.dohUrl})`,
  );
  logInfo(
    "System",
    `连接池: Min=${CONFIG.minPoolSize}, Max=${CONFIG.maxPoolSize}`,
  );
  ...
  logInfo(
    "System",
    `Metrics: ${CONFIG.enableMetrics ? `开启 (端口:${CONFIG.metricsPort})` : "关闭"}`,
  );
  logInfo("System", `连接池预热: ${CONFIG.enablePoolWarmup ? "开启" : "关闭"}`);
  logInfo(
    "System",
    `断线重连: ${CONFIG.enableAutoReconnect ? "开启" : "关闭"}`,
  );
  logInfo(
    "System",
    `动态池调整: ${CONFIG.enableDynamicPool ? "开启" : "关闭"}`,
  );
  logInfo("System", "----------------------------------------");
